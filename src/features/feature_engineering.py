...
class FeatureEngineering:
    ...
    def engineer_features(self):
        ...
        try:
            ...
            logging.info(f"\nRUL range: {self.cleaned_data['rul_hours'].min():.1f} — {self.cleaned_data['rul_hours'].max():.1f}")
            ...
            logging.info("vibration_magnitude computed")
            ...
            logging.info(f"After lag features: {len(self.cleaned_data):,} rows")
            logging.info(f"Lag/roll features added: {len(LAG_FEATURES)}")
            logging.info(f"Total columns: {self.cleaned_data.shape[1]}")
            ...
            logging.info(f"Rows: {len(rul_dataset):,}")
